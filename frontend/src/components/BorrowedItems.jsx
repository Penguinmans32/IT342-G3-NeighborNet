...
const BorrowedItems = () => {
  ...
  const fetchBorrowedItems = async () => {
    try {
      setLoading(true)
      const response = await fetch("https://neighbornet-back-production.up.railway.app/api/borrowing/items/borrowed", {
        headers: {
    ...
    const fetchReturnStatuses = async () => {
      try {
        ...
        for (const item of borrowedItems) {
          const response = await fetch(`https://neighbornet-back-production.up.railway.app/api/borrowing/returns/status/${item.id}`, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
          });
