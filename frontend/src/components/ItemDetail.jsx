...
const ItemDetail = () => {
  ...
  const confirmDelete = async () => {
    setIsDeleting(true);
    try {
      await axios.delete(`https://it342-g3-neighbornet.onrender.com/api/borrowing/items/user/${id}`, {
  ...
  const handleUpdate = async () => {
    try {
        ...
        const response = await axios.put(
            `https://it342-g3-neighbornet.onrender.com/api/borrowing/items/${id}`,
            updateRequest,
            {
                headers: {
  ...
  useEffect(() => {
    const fetchItemDetails = async () => {
      try {
        const response = await axios.get(`https://it342-g3-neighbornet.onrender.com/api/borrowing/items/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        })
        setItem(response.data)
  ...
  const handleBorrowSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`https://it342-g3-neighbornet.onrender.com/api/borrowing/requests`, formattedRequest, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
          "Content-Type": "application/json",
        },
