...
const TreeNode = ({ classItem, index, onExpand, isExpanded, onDelete, onEdit, thumbnailUrl }) => {
  ...
  useEffect(() => {
    const fetchLessons = async () => {
      try {
        const response = await axios.get(`https://it342-g3-neighbornet.onrender.com/api/classes/${classItem.id}/lessons`, {
          headers: {
...
const YourClasses = () => {
  ...
  const handleDeleteClass = useCallback(async (classId) => {
    ...
    if (await confirmDelete()) {
      try {
        const response = await axios.delete(`https://it342-g3-neighbornet.onrender.com/api/classes/${classId}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        })
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        const response = await axios.get("https://it342-g3-neighbornet.onrender.com/api/classes/my-classes", {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        })
